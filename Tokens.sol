...
contract Pandemic is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    constructor (address routerAddress)  {
       ...
       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
        ...
        _teamWallet = ; //Add team wallet address here
    }
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= MaxSupply, "PDC: cap exceeded");
        _mint(to, amount);
    }
    ...
    //     require(_isExcluded[account] != status, "PDC: Account already have same status");
    ...
    function setAutomatedMarketMaker(address AMM, bool status) public onlyOwner {
        require(_isAutomatedMarketMaker[AMM] != status, "PDC: AMM already have given status");
        require(AMM != uniswapV2Pair, "PDC: Cannot Remove uniswap V2 pair from Automated Market Maker");
    ...
    //     require(newBuyFee <= 20, "PDC: Fee cannot be set more than 20%");
    //     _buyFee = newBuyFee;
    //     emit BuyFeeUpdated(newBuyFee);
    // }

    // function setSellFee(uint256 newSellFee) public onlyOwner {
    //     require(newSellFee <= 20, "PDC: Buy Fee cannot be set more than 20%");
    //     _sellFee = newSellFee;
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "PDC: approve from the zero address");
        require(spender != address(0), "PDC: approve to the zero address");
    ...
    //     require(amount > 0, "PDC: Transfer amount must be greater than zero");
    ...
    function _transfer(
        ...
        require(from != address(0), "PDC: transfer from the zero address");
        require(to != address(0), "PDC: transfer to the zero address");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "PDC: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "PDC: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "PDC: burn from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "PDC: burn amount exceeds balance");
        unchecked {
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "PDC: insufficient allowance");
            unchecked {
